(function($){
	...
	var analystSlider = function(direction, count) {
		...
		var i_currentLeft = fixLeft($_sliderBox.position().left);
		var i_animateLeft;
		...
		if(direction === "left") {
			if((i_silder_width + i_currentLeft - i_viewBlock*i_unitWidth - i_unitWidth*count) < 0 ) {
				...
				$_sliderBox.css("left", (i_currentLeft + i_unitWidth*count) + "px");
		...
		b_canAnimate = false;
		$_sliderBox.animate({left: i_animateLeft +'px'}, "slow","", function(){
			b_canAnimate = true;
	...
	$(function(){
		...
		$(".turnpage_handle span").click(function(){
			...
			if(i_num > i_currentNum)
			{
				analystSlider("left", i_num - i_currentNum );
			}
			else
			{
				analystSlider("right", i_currentNum - i_num );
			}
